...
class Watcher(object):
    ...
    def __init__(self, name, cmd, args=None, numprocesses=1, warmup_delay=0.,
        ...
        print('***pprint(self.stdout_stream)***')
        pprint(self.stdout_stream)
        self.stderr_stream = get_stream(self.stderr_stream_conf)
        print('***pprint(self.stderr_stream)***')
        pprint(self.stderr_stream)
        ...
        self.optnames = (("numprocesses", "warmup_delay", "working_dir",
                          ...
                          "close_child_stderr", "use_papa", "stdout_stream") +
                         tuple(options.keys()))
    ...
    def pending_socket_event(self):
        logger.debug('pending_socket_event:%s, %s, %s' % (self.on_demand, self.arbiter.socket_event, self.on_demand and not self.arbiter.socket_event))
    ...
    def manage_processes(self):
        ...
        if len(self.processes) < self.numprocesses and not self.is_stopping():
            if self.respawn:
                ...
                logger.debug('manage_processes, yield self._stop')
                yield self._stop()
    ...
    def spawn_processes(self):
        ...
        if self.pending_socket_event:
            self._status = "stopped"
            logger.debug('spawn_processes: pending_socket_event')
            return
    ...
    def _start(self):
        ...
        """
        if self.pending_socket_event:
            logger.debug('_start: pending_socket_event %s', str(self))
            return
    ...
    def set_opt(self, key, val):
        ...
        """
        logger.debug("set_opt(%s, %s)" % (key, val))
        action = 0
    ...
    def options(self, *args):
        ...
        for name in sorted(self.optnames):
            ...
            if name in self._options:
                ...
                # some of the options are FileStream classes/subclasses
                # add the 'name' of the class to the options, not the class
                # itself
                if (issubclass(type(getattr(self, name)), FileStream)):
                    # ignore std[out|err|in]_stream as these are actual classes holding
                    # data, we can show the 'class' name, but can't read the data back
